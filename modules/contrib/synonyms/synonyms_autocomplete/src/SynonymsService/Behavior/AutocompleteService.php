...
class AutocompleteService implements BehaviorInterface, WidgetInterface {
   ...
   */
  public function __construct(KeyValueFactoryInterface $key_value, SelectionPluginManagerInterface $selection_plugin_manager, Connection $database, EntityTypeManagerInterface $entity_type_manager) {
    ...
    if ($this->keyValue->has($key_value_key)) {
      ...
      if (count($suggestions) < $settings['suggestion_size']) {
        foreach ($target_bundles as $target_bundle) {
          if (\Drupal::getContainer()->get('synonyms.behavior_service')->serviceIsEnabled($settings['target_type'], $target_bundle, $this->getId())) {
            foreach (\Drupal::getContainer()->get('synonyms.provider_service')->getSynonymConfigEntities($settings['target_type'], $target_bundle) as $synonym_config) {
